...
def compute_color (prob_table):
    """ Choose color number from probability range. """
    p = n = 0
...
class Codechunk:
    ...
    def __init__ (self, x, y, erasing_y, color,
...
class Thr (threading.Thread):
    """ Thread responsible for displaying stuff... """
    ...
    def run (self):
        ...
        # init tables (lists)
        self.color_table = [curses.color_pair (10) | curses.A_BOLD,
                            curses.color_pair (0)  | curses.A_BOLD,
                            curses.color_pair (2) | curses.A_BOLD,
                            curses.color_pair (6) | curses.A_BOLD,
                            curses.color_pair (8) | curses.A_BOLD,
                            curses.color_pair (9) | curses.A_BOLD,]
        self.head_prob_table = [MC.GREEN_HEAD_PROBABILITY,
        ...
        for i in range (MC.COLS):
            falling_speed = (random.randrange (MC.FALLING_MINSPEED,
                                               MC.FALLING_MAXSPEED)) * 0.01
            self.cc = [Codechunk (i,
                                  ...
                                  falling_speed,
                                  falling_speed - 0.20,
                                  random.randrange (2, MC.HEAD_MAXLEN))] + self.cc
        ...
        while (not self.stop_this_thread):
            ...
            for cci in self.cc:
                # "erasing tail"
                if cci.erasing_y >= MC.MATRIX_START_LINE  and \
                   cci.erasing_y < MC.MATRIX_END_LINE:
                    try:
                        self.scr.addch (int (math.floor (cci.erasing_y)), cci.x,
                                        ' ', cci.color)
                    except:
                        pass
                # draw "tail"
                if cci.y - cci.head_length >= MC.MATRIX_START_LINE  and \
                   cci.y - cci.head_length  < MC.MATRIX_END_LINE:
                    try:
                        self.scr.addch (int (math.floor (cci.y - cci.head_length)),
                                        cci.x,
                        ...
                        pass
                # draw head
                if cci.y >= MC.MATRIX_START_LINE  and  cci.y < MC.MATRIX_END_LINE:
                    try:
                        self.scr.addch (int (math.floor (cci.y)), cci.x,
                                        chr (random.randrange (30, 120)),
                                        cci.color)
                    except:
                        pass
                ...
                # if codechunk falls, reset (random) it's attributes
                if cci.erasing_y >= MC.MATRIX_END_LINE:
                    # reset y positions
                    cci.y = MC.MATRIX_START_LINE
                    cci.erasing_y = MC.MATRIX_START_LINE - (random.randrange (0, MC.LINES))
                    # randomize colors
                    cci.color = self.color_table[compute_color (self.head_prob_table)]
                    cci.trail_color = cci.color
                    # randomize falling and erasing speeds
                    cci.falling_speed = (random.randrange (MC.FALLING_MINSPEED,
                                                           MC.FALLING_MAXSPEED)) * 0.01
